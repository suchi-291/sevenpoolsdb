...
export default class FuturesBinBot extends Component {
  constructor(props) {
    super(props);
    this.state = {
      buttons: {
  ...
  componentDidMount() {
    ...
    const savedState = localStorage.getItem("futuresBinanceState");
    if (savedState) {
      // If state exists in localStorage, set it to this.state
      this.setState(JSON.parse(savedState));
      console.log("futures component mounted");
    }
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState !== this.state) {
      localStorage.setItem("futuresBinanceState", JSON.stringify(this.state));
    }
  }
        ...
        <div className={`db-bot-table-nav-bottom ${theme}`}>
          ...
          {showPlatform && (
            <div className="db-platform-name-wrap">
              <p className={`db-platform-p ${theme}`}>Platform: Binance</p>
            </div>
          )}
          {!showInvestment && !showPlatform && (
            <div className="db-bot-header">
              <p className={`db-bot-header-p ${theme}`}>
                Hi there! Welcome to 7 Pools Futures market place on Binance
              </p>
            </div>
          )}
